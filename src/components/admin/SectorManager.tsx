...
export default function SectorManager() {
  ...
  const inputCls =
    'border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:ring-2 focus:ring-cactus-accent/30';
  ...
  const startEdit = (s: Sector) => {
    setEditing(s);
    setForm({ name: s.name, color: s.color, iconName: s.iconName });
    setCreating(false);
  };
  ...
  const save = () => {
    ...
    if (creating) {
      addSector({ id: generateId(), ...form });
    } else if (editing) {
      updateSector({ ...editing, ...form });
    }
    cancel();
  };
